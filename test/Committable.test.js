...
describe('Committable', function () {
    context('with deployed contracts', function () {
        ...
        context("[permit] function test", function () {
            beforeEach("mint a token to signer", async function () {
                /* sign some tokenId */
                let abiCoder = new ethers.utils.AbiCoder;
                let signature_0 = await signer.signMessage(ethers.utils.arrayify(abiCoder.encode(['uint256'], [tokenId_0])));
                /* mint tokenId_0 to signer */
                await committable.mint(signer.address, tokenId_0, signature_0);
            })
            it("should successful approve using permit with legitimate sig", async function () {
                /* approve user to use tokenId-0 */
                nonce = await committable.nonces(signer.address);
                let permit_sig = await signer.signMessage(ethers.utils.arrayify(hashPermit(user.address, tokenId_0, nonce, 0)));
                await committable.connect(user).permit(user.address, tokenId_0, 0, permit_sig);
                expect(await committable.getApproved(tokenId_0)).to.equal(user.address);
            })
            it("should successful approve using permit with legitimate sig and accumulating nonces", async function () {
                /* approve user to use tokenId-0 */
                nonce = await committable.nonces(signer.address);
                {
                    let permit_sig = await signer.signMessage(ethers.utils.arrayify(hashPermit(user.address, tokenId_0, nonce, 0)));
                    await committable.connect(user).permit(user.address, tokenId_0, 0, permit_sig);
                    expect(await committable.getApproved(tokenId_0)).to.equal(user.address);
                    await committable.approve(ZERO_ADDRESS, tokenId_0);
                    expect(await committable.getApproved(tokenId_0)).to.equal(ZERO_ADDRESS);
                }
                nonce++;
                {
                    let permit_sig = await signer.signMessage(ethers.utils.arrayify(hashPermit(user.address, tokenId_0, nonce, 0)));
            ...
            it("should revert permit with malicious sig", async function () {
                /* approve user to use tokenId-0 */
                nonce = await committable.nonces(signer.address);
                let permit_sig = await user.signMessage(ethers.utils.arrayify(hashPermit(user.address, tokenId_0, nonce, 0)));
                try {
                    await committable.connect(user).permit(user.address, tokenId_0, 0, permit_sig);
                    throw null;
                } catch (err) {
            ...
            it("should revert permit with expired sig", async function () {
                /* approve user to use tokenId-0 */
                nonce = await committable.nonces(signer.address);

                let permit_sig = await signer.signMessage(ethers.utils.arrayify(hashPermit(user.address, tokenId_0, nonce, 1)));
                try {
                    await committable.connect(user).permit(user.address, tokenId_0, 1, permit_sig);
                    throw null;
                } catch (err) {
