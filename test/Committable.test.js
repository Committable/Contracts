...
describe('Committable', function () {
    ...
    context('with deployed contracts', function () {
        ...
        it('should return signer', async function () {
            expect(await tokenProxy.getSigner()).to.equal(signer.address);
            expect(await controller['signer()']()).to.equal(signer.address);
     

        })


        context('mint using legitimate commitInfo and signature', function () {
            beforeEach('mint tokens with legitimate commitInfo and signature', async function () {
